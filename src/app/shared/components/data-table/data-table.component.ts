...
export interface HeaderMapItem<T> {
  ...
  truncate?: boolean;
  sortable?: boolean;
  parseItem?: (value: unknown, item: T) => string;
}
...
@Component({
  ...
  template: `
    <div
      class="flex-none flex flex-col min-h-0 max-h-full bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden"
    >
      <div
        class="overflow-auto relative scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent"
      >
        @if (isLoading) {
                      ...
                      <ng-container
                        *ngTemplateOutlet="
                          ...
                          context: {
                            $implicit: item,
                            column: col,
                            value: getValue(item, col.keyItem),
                          }
                        "
                      ></ng-container>
      ...
      <!-- Pagination -->
      <div
        class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between shrink-0"
      >
...
export class DataTableComponent<T extends { id?: string }> {
  ...
  getValue(item: T, key: string): unknown {
    return key
      .split('.')
      .reduce((obj, k) => (obj as Record<string, unknown>)?.[k], item as unknown);
  }
  ...
  handleSort(col: HeaderMapItem<T>) {
    ...
    this.sortChange.emit({ orderBy, orderDirection: direction });
  }
